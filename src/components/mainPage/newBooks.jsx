...
const NewBooks = ({products}) => {
  ...
  const handleArrowLeftClick = () => {
    setSelectedProductIndex((prevIndex) => (prevIndex > 0 ? prevIndex - 1 : prevIndex));
  };
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const handleLike = () => {
      ...
      if (!liked) {
        ...
        const updatedLikedProducts = likedProducts.filter(product => product.uid !== currentProduct.uid);
    ...
    return(
        ...
        height="40"
            ...
            <p className='descriptionBooksNew'>{currentProduct.descriptionSe}</p>
...
<div className='likeProductSmallSaleNovu' onClick={() => addToCart(currentProduct.uid)}>
<HandySvg 
...
<div  className={`likeProductSmallSaleNovuGr${liked ? 'Click' : ''}`} onClick={handleLike}>
<HandySvg 
                    src={iconSrcWhite}
                    width="34"
        height="31"
