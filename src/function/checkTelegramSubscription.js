...
//         alert('Користувач не підписаний')
//         // Виконати дію, яку ви бажаєте, коли виникає помилка зі статусом 400
//         // Наприклад, вивести повідомлення або виконати додаткові кроки обробки помилки
//       } else {
//         console.error('Інша помилка при виклику Telegram Bot API:', error);
//       }
  ...
  try {
    ...
    if (response.data.ok && response.data.result && response.data.result.status === 'member') {
      ...
      if (userDoc.exists()) {
        ...
        if (discount === 0) {
          await updateDoc(userRef, { discount: 5, category: 'Член "Клубу Майстрів"', signed:'true' });
    ...
    if (error.response && error.response.status === 400) {
      ...
      if (userDoc.exists()) {
        ...
        if (discount === 5) {
          await updateDoc(userRef, { discount: 0, category: 'Читач', signed: 'false' });
